...
class PayloadModule {
    constructor(domain) {
        this.createProvider = (session, data) => {
            const payload = {};
        ...
        this.buildContext = (session, action) => {
            const contextConfig = [
                {
                    beckn_key: "bap_id",
            ...
            contextConfig.map((item) => {
                try {
                    if (eval(item.value) && (item.check ? eval(item.check) : true))
                        this.createNestedField(context, item.beckn_key, item.compute ? eval(item.compute) : eval(item.value));
                }
                catch (err) {
        ...
        this.createNestedField = (obj, path, value) => {
